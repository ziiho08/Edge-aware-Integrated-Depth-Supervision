...
PSEUDODEPTH_COMPATIBLE_LOSSES = (DepthLossType.SPARSENERF_RANKING, DepthLossType.silog_loss)
...
def depth_loss(
    ...
    if depth_loss_type == DepthLossType.silog_loss:
        return silog_loss(termination_depth, predicted_depth)
...
def silog_loss(rendered_depth, gt_depth):

    if rendered_depth.shape[0] % 2 != 0:
        rendered_depth = rendered_depth[:-1, :]
        gt_depth = gt_depth[:-1, :]
    variance_focus = 1
    d = torch.log(rendered_depth) - torch.log(gt_depth)
