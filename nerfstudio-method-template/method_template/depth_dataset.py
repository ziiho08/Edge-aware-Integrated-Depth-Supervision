...
class DepthDataset(InputDataset):
    ...
    def __init__(self, dataparser_outputs: DataparserOutputs, scale_factor: float = 1.0):
        ...
        if len(dataparser_outputs.image_filenames) > 0 and (
            ...
            dataparser_outputs.metadata["lidar_filenames"] = None
            dataparser_outputs.metadata["depth_unit_scale_factor"] = 1.0
            self.metadata["depth_filenames"] = None
            self.metadata["lidar_filenames"] = None
            self.metadata["depth_unit_scale_factor"] = 1.0

        self.depth_filenames = self.metadata["depth_filenames"]
        self.lidar_filenames = self.metadata["lidar_filenames"]
        self.depth_unit_scale_factor = self.metadata["depth_unit_scale_factor"]

    def get_metadata(self, data: Dict) -> Dict:
        if self.depth_filenames is None:
            return {"depth_image": self.depths[data["image_idx"]], "lidar_gt": self.depths[data["image_idx"]]}

        filepath = self.depth_filenames[data["image_idx"]]
        lidar_filepath = self.lidar_filenames[data["image_idx"]]
        ...
        lidar_image = get_depth_image_from_path(
            filepath=lidar_filepath, height=height, width=width, scale_factor=scale_factor
        )

        return {"depth_image": depth_image, "lidar_gt": lidar_image}
