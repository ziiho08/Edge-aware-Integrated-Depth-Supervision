...
from method_template.template_datamanager import (
    TemplateDataManager,TemplateDataManagerConfig)
...
@dataclass
class TemplateDataParserConfig(NerfstudioDataParserConfig):
    ...
    depth_unit_scale_factor: float = 1/335
    depth_type: float = "sparse"
    orientation_method: Literal["pca", "up", "vertical", "none"] = "none"
    """The method to use for orientation."""
    center_method: Literal["poses", "focus", "none"] = "none"
    """The method to use to center the poses."""

    _target: Type = field(default_factory=lambda: Template)

class Template(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.data.suffix == ".json":
            ...
            meta = load_from_json(self.config.data / f"transforms_{self.config.depth_type}.json")
        ...
        for frame in frames:
            ...
            if "lidar_file_path" in frame:
                lidar_filepath = Path(frame["lidar_file_path"])
                lidar_fname = self._get_fname(lidar_filepath, data_dir, downsample_folder_prefix="lidars_")
                lidar_filenames.append(lidar_fname)
        ...
        """
        assert len(lidar_filenames) == 0 or (len(lidar_filenames) == len(image_filenames)), """
        Different number of image and depth filenames.
        You should check that lidar_file_path is specified for every frame (or zero frames) in transforms.json.
        """
        ...
        lidar_filenames = [lidar_filenames[i] for i in indices] if len(lidar_filenames) > 0 else []
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            metadata={
                ...
                "lidar_filenames": lidar_filenames if len(lidar_filenames) > 0 else None,
    ...
    def _get_fname(self, filepath: Path, data_dir: Path, downsample_folder_prefix="images_") -> Path:
        ...
        """
        if downsample_folder_prefix in ["images_", "masks_"]:
